...
def test_unknown_paths_are_a_json_404_not_the_index_page(client, dist):
    """No router, so no deep links: a typo'd API path must not return HTML 200."""
    res = client.get('/music/track')
...
def test_non_get_on_an_unknown_path_is_still_a_404(client, dist):
    ...
    res = client.put('/todo/abc', json={'task': 'x'})
